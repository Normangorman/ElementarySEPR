        ...
        if (Application.isPlaying == false)
        {
            ...
            else
            {
                if (newElementName == false && renameElementName == false && deleteElementName == false)
                {
                    ...
                    if (UIManager.GetIndexForElementName(GetUIElement.elementName) == -1)
                    {
                        DoozyUIRedundancyCheck.UIElementRedundancyCheck(GetUIElement);
                    }
                    elementNameCurrentIndex = UIManager.GetIndexForElementName(GetUIElement.elementName);
                    elementNameCurrentIndex = EditorGUILayout.Popup(elementNameCurrentIndex, elementNames, GUILayout.Width(182));
                    GetUIElement.elementNameReference = UIManager.GetDoozyUIData.elementNames[elementNameCurrentIndex]; //we reference the class not the value (we need a reference)
                ...
                if (newElementName == false && renameElementName == false && deleteElementName == false)
                {
                    if (GetUIElement.elementName.Equals(UIManager.DEFAULT_ELEMENT_NAME))
                    {
                ...
                else if (newElementName == true)
                {
                    ...
                    if (GUILayout.Button("confirm", GUILayout.Width(89), GUILayout.Height(16)))
                    {
                        ...
                        GetUIElement.elementNameReference = UIManager.GetDoozyUIData.elementNames[UIManager.GetIndexForElementName(tempElementNameString)];    //we update the reference
                ...
                else if (deleteElementName == true)
                {
                    ...
                    {
                        if (GetUIElement.elementNameReference.elementName.Equals(UIManager.DEFAULT_ELEMENT_NAME) == false)   //we delete the entry only if it's not the first one or the default one, that we are keeping as an empty string entry
                        {
                            ...
                            GetUIElement.elementNameReference = UIManager.GetDoozyUIData.elementNames[elementNameCurrentIndex]; //we update the elementName reference
                            GetUIElement.elementName = GetUIElement.elementNameReference.elementName; //we save the backup
                            DoozyUIRedundancyCheck.CheckAllTheUIElements();
                        }
                        else
        ...
        else
        {
            ...
            if (GUILayout.Button("SHOW", GUILayout.Width(206), GUILayout.Height(24))) //this is the SHOW UIELement in PlayMode button
            {
                if (GetUIElement.AreInAnimationsEnabled)
                {
                    ...
                    {
                        UIManager.ShowUiElement(GetUIElement.elementNameReference.elementName, false); //we play the IN animations
                    }
                }
                else
                {
            ...
            if (GUILayout.Button("HIDE", GUILayout.Width(206), GUILayout.Height(24))) //this is the HIDE UIElement in PlayMode button
            {
                if (GetUIElement.AreOutAnimationsEnabled)
                {
                    ...
                    {
                        UIManager.HideUiElement(GetUIElement.elementNameReference.elementName, false); //we play the OUT animations
                    }
                }
                else
                {
