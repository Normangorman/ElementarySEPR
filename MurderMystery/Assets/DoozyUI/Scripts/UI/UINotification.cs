...
namespace DoozyUI
{
    ...
    public class UINotification : MonoBehaviour
    {
        ...
        //private bool didThisNotificationDisableTheBackButton = false; //old method to disable the back button (before the additive bool)
        #endregion
        ...
        void Awake()
        {
            ...
            if (UIManager.backButtonDisabled == false) //we check if the 'Back' button is enabled; if enabled, we disable it and we re-enable it when this notification closes
            {
                UIManager.DisableBackButton(); //we disable the 'Back' button
                didThisNotificationDisableTheBackButton = true; //we mark that we did this toggle so that we can change it back to the previous state when the notification closes
            }
            */
