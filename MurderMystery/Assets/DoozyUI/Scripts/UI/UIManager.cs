...
namespace DoozyUI
{
    ...
    public class UIManager : Singleton<UIManager>
    {
        ...
        public const string VERSION = "Version 2.5";
        #endregion
        ...
        private static void OnButtonClick(UIButtonMessage m)
        {
            ...
            if (m.showElements != null && m.showElements.Count > 0)
            {
                for (int i = 0; i < m.showElements.Count; i++)
                {
                    ShowUiElement(m.showElements[i]);
                }
            }

            if (m.hideElements != null && m.hideElements.Count > 0)
            {
                for (int i = 0; i < m.hideElements.Count; i++)
                {
                    HideUiElement(m.hideElements[i]);
                }
            }

            if (m.addToNavigationHistory)
            {
                Navigation navItem = new Navigation();
                navItem.showElements = new List<string>();
                navItem.showElements = m.hideElements;
                navItem.hideElements = new List<string>();
                navItem.hideElements = m.showElements;
        ...
        public static void RenameElementName(int index, string newName)
        {
            ...
            if (tempElementsArray != null && tempElementsArray.Length > 0)
            {
                ...
                {
                    if (tempElementsArray[i].elementNameReference.elementName.Equals(previousName))
                    {
        ...
        public static void RegisterUiElement(UIElement element)
        {
            ...
            if (uiElementRegistry.ContainsKey(element.elementNameReference.elementName)) //we check if the dictionary has the key (the element's elementName)
            {
                if (uiElementRegistry[element.elementNameReference.elementName].Contains(element) == false) //because the registry has the key (the element's elementName), we check that the element is not already registered to the registry with that element (the reference)
                {
                    uiElementRegistry[element.elementNameReference.elementName].Add(element); //we add the element reference with the elementName key
                }
            }
            else
            {
                uiElementRegistry.Add(element.elementNameReference.elementName, new List<UIElement>() { element }); //because the registry does not contain the elementName key, we add it and we also add the value (the element reference)
            }
        }
        ...
        public static void UnregisterUiElement(UIElement element)
        {
            ...
            if (uiElementRegistry.ContainsKey(element.elementNameReference.elementName))
            {
                uiElementRegistry[element.elementNameReference.elementName].Remove(element);
            }
        }
        ...
        {
            if (effect == null || effect.targetUIElement == null || string.IsNullOrEmpty(effect.targetUIElement.elementNameReference.elementName))
                return;

            if (uiEffectRegistry == null)
                uiEffectRegistry = new Dictionary<string, List<UIEffect>>();

            if (uiEffectRegistry.ContainsKey(effect.targetUIElement.elementNameReference.elementName))
            {
                uiEffectRegistry[effect.targetUIElement.elementNameReference.elementName].Add(effect);
            }
            else
            {
                uiEffectRegistry.Add(effect.targetUIElement.elementNameReference.elementName, new List<UIEffect>() { effect });
            }
        }
        ...
        public static void UnregisterUiEffect(UIEffect effect)
        {
            ...
            if (uiEffectRegistry.ContainsKey(effect.targetUIElement.elementNameReference.elementName))
            {
                uiEffectRegistry[effect.targetUIElement.elementNameReference.elementName].Remove(effect);
            }
        }
        ...
        private static void LoadNotification(UINotification.NotificationData nData)
        {
            ...
            try
            {
                //we look for the notification prefab; we do this in a 'try catch' just in case the name was mispelled or the prefab does not exist
                prefab = Resources.Load(nData.prefabName);
            }
            catch (UnityException e)
            {
                Debug.Log("[DoozyUI] [SetupNotification]: " + e);
            }

            if (prefab == null)
            {
                Debug.Log("[DoozyUI] [SetupNotification]: " + nData.prefabName + " prefab does not exist or is not located in a Resources folder");
                return;
            }

            GameObject notification = Instantiate(Resources.Load(nData.prefabName, typeof(GameObject)), GetUiContainer.transform.position, Quaternion.identity) as GameObject;
        ...
        /// <summary>
        /// This will show a premade notification with the given settings.
        /// </summary>
        ...
        #region Methods for Pause and UI Navigation Management - TogglePause, ApplicationQuit, BackButtonEvent, DisableBackButton, EnableBackButton
