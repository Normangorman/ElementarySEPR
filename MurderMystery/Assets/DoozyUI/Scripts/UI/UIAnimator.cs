...
namespace DoozyUI
{
    public class UIAnimator : MonoBehaviour
    {
        ...
        {
            public Vector3 startAnchoredPosition = Vector3.zero;
        ...
        public static void ResetRectTransform(RectTransform rectTransform, InitialData initialData, ResetType resetType = ResetType.All)
        {
            ...
            {
                case ResetType.All:
                    rectTransform.anchoredPosition = initialData.startAnchoredPosition;
                    rectTransform.rotation = Quaternion.Euler(initialData.startRotation);
        ...
        public static void DoMoveIn(MoveIn moveIn, RectTransform rectTransform, InitialData initialData, bool instantAction)
        {
            ...
            Vector3 targetPosition = initialData.startAnchoredPosition;
            ...
            switch (moveIn.moveFrom)
            {
                case MoveDetails.ParentPosition:
                    ...
                    targetPosition = new Vector2(rectTransform.parent.GetComponent<RectTransform>().anchoredPosition.x + moveIn.positionAdjustment.x, rectTransform.parent.GetComponent<RectTransform>().anchoredPosition.y + moveIn.positionAdjustment.y);
                    break;
                ...
                case MoveDetails.LocalPosition:
                    ...
                    targetPosition = new Vector2(moveIn.positionFrom.x + moveIn.positionAdjustment.x, moveIn.positionFrom.y + moveIn.positionAdjustment.y);
                    break;

                case MoveDetails.TopScreenEdge:
                    targetPosition = new Vector2(initialData.startAnchoredPosition.x + moveIn.positionAdjustment.x, initialData.startAnchoredPosition.y + UIManager.GetUIScreenRect.size.y + moveIn.positionAdjustment.y);
                    break;

                case MoveDetails.RightScreenEdge:
                    targetPosition = new Vector2(initialData.startAnchoredPosition.x + UIManager.GetUIScreenRect.size.x + moveIn.positionAdjustment.x, initialData.startAnchoredPosition.y + moveIn.positionAdjustment.y);
                    break;
                ...
                case MoveDetails.TopLeft:
                    targetPosition = new Vector2(-UIManager.GetUIScreenRect.size.x + moveIn.positionAdjustment.x, UIManager.GetUIScreenRect.size.y + moveIn.positionAdjustment.y);
                    break;

                case MoveDetails.TopCenter:
                    targetPosition = new Vector2(0 + moveIn.positionAdjustment.x, UIManager.GetUIScreenRect.size.y + moveIn.positionAdjustment.y);
                    break;
                ...
                case MoveDetails.MiddleLeft:
                    targetPosition = new Vector2(-UIManager.GetUIScreenRect.size.x + moveIn.positionAdjustment.x, 0 + moveIn.positionAdjustment.y);
                    break;

                case MoveDetails.MiddleCenter:
                    targetPosition = new Vector2(0 + moveIn.positionAdjustment.x, 0 + moveIn.positionAdjustment.y);
                    break;
        ...
        public static void DoRotationIn(RotationIn rotationIn, RectTransform rectTransform, InitialData initialData, bool instantAction)
        {
            ...
            if (instantAction == false) //if this is not an instantAction, then we use a tween
            {
                ...
                rectTransform
                .DORotate(rotationIn.rotateFrom, time, RotateMode.FastBeyond360)
                    .SetDelay(delay)
            ...
            {
                rectTransform.Rotate(rotationIn.rotateFrom);
            }
        }
        #endregion
        ...
        public static void DoMoveLoop(MoveLoop moveLoop, RectTransform rectTransform, InitialData initialData)
        {
            ...
            Vector3 animBeginPosition = new Vector3(initialData.startAnchoredPosition.x - moveLoop.movement.x,
                                                    initialData.startAnchoredPosition.y - moveLoop.movement.y,
                                                    initialData.startAnchoredPosition.z - moveLoop.movement.z);

            Vector3 animEndPosition = new Vector3(initialData.startAnchoredPosition.x + moveLoop.movement.x,
                                                   initialData.startAnchoredPosition.y + moveLoop.movement.y,
                                                   initialData.startAnchoredPosition.z + moveLoop.movement.z);
        ...
        public static void DoRotationLoop(RotationLoop rotationLoop, RectTransform rectTransform, InitialData initialData)
        {
            ...
            {
                rectTransform.DORotate(animBeginRotation, rotationLoop.time / 2f, RotateMode.Fast)
                   ...
                   .OnComplete(() =>
                   {
                       rectTransform.DORotate(animEndRotation, rotationLoop.time, RotateMode.Fast)
                               .SetEase(rotationLoop.easeType)
            ...
            {
                rectTransform.DORotate(rotationLoop.rotation, rotationLoop.time, RotateMode.FastBeyond360)
                  .SetRelative(true)
                  .SetDelay(rotationLoop.delay)
                  .SetLoops(rotationLoop.loops, rotationLoop.loopType)
                  .SetEase(rotationLoop.easeType)
        ...
        public static void DoMoveOut(MoveOut moveOut, RectTransform rectTransform, InitialData initialData, bool instantAction)
        {
            ...
            switch (moveOut.moveTo)
            {
                case MoveDetails.ParentPosition:
                    ...
                    targetPosition = new Vector2(rectTransform.parent.GetComponent<RectTransform>().anchoredPosition.x + moveOut.positionAdjustment.x, rectTransform.parent.GetComponent<RectTransform>().anchoredPosition.y + moveOut.positionAdjustment.y);
                    break;
                ...
                case MoveDetails.LocalPosition:
                    ...
                    targetPosition = new Vector2(moveOut.positionTo.x + moveOut.positionAdjustment.x, moveOut.positionTo.y + moveOut.positionAdjustment.y);
                    break;

                case MoveDetails.TopScreenEdge:
                    targetPosition = new Vector2(initialData.startAnchoredPosition.x + moveOut.positionAdjustment.x, initialData.startAnchoredPosition.y + UIManager.GetUIScreenRect.size.y + moveOut.positionAdjustment.y);
                    break;

                case MoveDetails.RightScreenEdge:
                    targetPosition = new Vector2(initialData.startAnchoredPosition.x + UIManager.GetUIScreenRect.size.x + moveOut.positionAdjustment.x, initialData.startAnchoredPosition.y + moveOut.positionAdjustment.y);
                    break;
                ...
                case MoveDetails.TopLeft:
                    targetPosition = new Vector2(-UIManager.GetUIScreenRect.size.x + moveOut.positionAdjustment.x, UIManager.GetUIScreenRect.size.y + moveOut.positionAdjustment.y);
                    break;

                case MoveDetails.TopCenter:
                    targetPosition = new Vector2(0 + moveOut.positionAdjustment.x, UIManager.GetUIScreenRect.size.y + moveOut.positionAdjustment.y);
                    break;
                ...
                case MoveDetails.MiddleLeft:
                    targetPosition = new Vector2(-UIManager.GetUIScreenRect.size.x + moveOut.positionAdjustment.x, 0 + moveOut.positionAdjustment.y);
                    break;

                case MoveDetails.MiddleCenter:
                    targetPosition = new Vector2(0 + moveOut.positionAdjustment.x, 0 + moveOut.positionAdjustment.y);
                    break;
        ...
        public static void DoMoveOutAnimation(Vector2 position, MoveOut moveOut, float time, float delay, RectTransform rectTransform, InitialData initialData, bool instantAction)
        {
            if (instantAction == false) //if this is not an instantAction, then we use a tween
            {
                ...
                rectTransform
                    .DOAnchorPos(position, time, false)
                        .SetDelay(delay)
        ...
        public static void DoRotationOut(RotationOut rotationOut, RectTransform rectTransform, InitialData initialData, bool instantAction)
        {
            ...
            if (instantAction == false) //if this is not an instantAction, then we use a tween
            {
                ...
                rectTransform
                    .DORotate(rotationOut.rotateTo, time, RotateMode.FastBeyond360)
                        .SetDelay(delay)
            ...
            {
                rectTransform.Rotate(rotationOut.rotateTo);
            }
        }
        #endregion
        ...
        public static void StartOnClickAnimations(RectTransform rectTransform, InitialData initialData, UIAnimationManager.OnClickAnimations onClickAnimSettings)
        {
            if (onClickAnimSettings.punchPositionEnabled)
            {
                rectTransform.DOPunchAnchorPos(onClickAnimSettings.punchPositionPunch, onClickAnimSettings.punchPositionDuration, onClickAnimSettings.punchPositionVibrato, onClickAnimSettings.punchPositionElasticity, onClickAnimSettings.punchPositionSnapping)
                    ...
                    .OnComplete(() =>
                    {
                        rectTransform.DOAnchorPos3D(initialData.startAnchoredPosition, 0.1f).Play();
                    })
                    .Play();
            }
        ...
        {
            if (rectTransform == null || rectTransform.GetComponent<UIButton>() == null)
                return;
